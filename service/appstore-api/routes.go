...
import (
	...
	"github.com/joshua-seals/gopherhelx/service/appstore-api/handlers"
)
...
func (app *Application) APIRouter() *chi.Mux {
	...
	router.Get("/{userId}/dashboard/session/{appId}-{session-id}", handlers.Session)

	router.Post("/{userId}/app/install/{appId}", handlers.AppInstall)
	router.Post("/{userId}/dashboard/start/{appId}", handlers.StartApp)
	router.Post("/{userId}/dashboard/stop/{appId}", handlers.StopApp)
...
func (app *Application) DebugRouter() http.Handler {
	...
	// And Instantiate with app variables.
